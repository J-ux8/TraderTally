...
- Properties 1.1, 1.2, and 1.3 can be combined into a single comprehensive property about local-first persistence
- Properties 2.1, 2.3 form the core idempotency property; 2.2 is implementation detail
- Properties 8.2, 8.3, 8.4, 8.5, 8.6 describe a state machine that can be tested as transitions
- Properties related to specific tables (13.2, 14.2, 15.2) are instances of the same general sync metadata property
- UI properties (10.1-10.7) are examples rather than universal properties
- Several properties about logging (12.2-12.5) can be combined into a general logging property

The following properties represent the unique, non-redundant correctness guarantees:

### Property 1: Local-First Persistence

*For any* user operation (create, update, delete), the operation SHALL succeed in local SQLite storage before any cloud synchronization is attempted, and SHALL return success to the UI only after local persistence completes.

**Validates: Requirements 1.1, 1.2, 1.3, 1.4**

### Property 2: Local Write Performance

*For any* single record write operation to local storage, the operation SHALL complete within 100 milliseconds.

**Validates: Requirements 1.5, 17.2**

### Property 3: Sync Idempotency (Round-Trip)

*For any* sync record, uploading the record to cloud storage multiple times with the same sync_version SHALL produce identical cloud state, and downloading then uploading then downloading SHALL produce an equivalent record.

**Validates: Requirements 2.1, 2.3, 2.5**

### Property 4: Sync Version Increment

*For any* sync record, the sync_version SHALL increment if and only if the record content changes locally.

**Validates: Requirements 2.4, 11.3**

### Property 5: Network Failure Resilience

*For any* sync operation that fails due to network error, the Sync_Engine SHALL increment retry_count, schedule a r